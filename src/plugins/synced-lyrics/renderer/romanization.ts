/*
 * Lyric romanization, split out of `utils.tsx` so its dictionaries (hanja,
 * pinyin-pro, tinyld, kuromoji, ...) are only evaluated when a lyric line
 * actually needs romanizing. Built eagerly they cost ~150 ms on every full page
 * load of the renderer bundle. Loaded through `romanize()` in `utils.tsx`.
 */
